...
#else
	      string result = displayedHeadword.data();
	      printf("got result %s\n", result.c_str());
#endif
              if ( result != headword )
...
main(int argc, char **argv)
{
  printf("argc %d\n", argc);
  if (argc < 2) {
    printf("usage %s <bglfile>\n", argv[0]);
    exit(-1);
  }

  printf("reading bgl %s\n", argv[1]);
  Babylon b(argv[1]);
  b.open();
  std::string str("");
  std::string tgt("");
  ...
  for( ; ; ) {
    ...
    printf("<p>word %s\ndisp %s\ndef %s\n", 
	   e.headword.c_str(), 
	   e.displayedHeadword.c_str(),
	   e.definition.c_str());
    //printf("trans %s part %s\n", e.trans.c_str(), e.part.c_str());
    std::vector<std::string>::iterator iter;
    for(iter=e.alternates.begin(); iter!=e.alternates.end(); ++iter) {
      printf("<p>  alt %s\n", (*iter).c_str());
    }
#endif
  }
